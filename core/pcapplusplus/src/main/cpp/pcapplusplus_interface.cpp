...
static pcpp::Packet rawPacketBytesToPacket(const uint8_t* packetBytes,
                                           int packetLength);
...
static jobject parseNetworkLayer(JNIEnv* env, const pcpp::Packet& packet) {
  jclass clazz = env->FindClass(
      "jp/co/casl0/android/simpleappblocker/core/pcapplusplus/model/"
      "NetworkLayer");
  if (clazz == nullptr) {
...
static jobject parseTransportLayer(JNIEnv* env, const pcpp::Packet& packet) {
  ...
  return env->NewObject(clazz, ctor, transportLayer.srcPort,
...
Java_jp_co_casl0_android_simpleappblocker_core_pcapplusplus_PcapPlusPlusInterface_analyzePacket(
  ...
  auto parsedPacket = rawPacketBytesToPacket(
      reinterpret_cast<const uint8_t*>(packetBytes), packetLength);
  ...
  jmethodID ctor = env->GetMethodID(
      clazz, "<init>",
      "(Ljp/co/casl0/android/simpleappblocker/core/pcapplusplus/model/"
      "NetworkLayer;Ljp/co/casl0/android/simpleappblocker/core/pcapplusplus/"
      "model/TransportLayer;)V");
  if (ctor == nullptr) {
    LOGE(TAG, "method not found");
    return nullptr;
  }

  return env->NewObject(clazz, ctor, parseNetworkLayer(env, parsedPacket),
 ...
 */
pcpp::Packet rawPacketBytesToPacket(const uint8_t* packetBytes,
                                    int packetLength) {
  ...
  pcpp::RawPacket rawPacket(packetBytes, packetLength, time, false,
                            pcpp::LINKTYPE_RAW);
...
std::string getTlsServerName(const pcpp::Packet& packet) {
  ...
  if (sslHandshakeLayer = packet.getLayerOfType<pcpp::SSLHandshakeLayer>();
      sslHandshakeLayer == nullptr) {
...
std::string getHttpHostName(const pcpp::Packet& packet) {
  ...
  if (httpRequestLayer = packet.getLayerOfType<pcpp::HttpRequestLayer>();
      httpRequestLayer == nullptr) {
